...
- **Secciones (cada una con layout DISTINTO para no repetir el patrón de "tarjetas/calugas"):** StickyHeader → Hero (top bar con logo HELI siempre visible + chips de energía + 1 máquina) → **FleetMarquee** (marquee infinito a todo el ancho, 8 máquinas) → TrustStrip (stats animadas `CountUp` + chips de certificación) → **EnergyBento** (bento asimétrico: hidrógeno destacado grande + 3 filas compactas) → **WhyHeli** (split editorial: heading sticky izq + filas con divisores, sin tarjetas) → **CTABanner** (franja roja full-width que rompe el ritmo) → **Categories** (nube de chips/pills, no tarjetas) → **Services** (3 columnas editoriales con números 01/02/03 + divisores verticales) → **Process** (línea de tiempo horizontal con nodos conectados) → **Quoter** (form) → Coverage (split + tiles de sucursales) → FAQ (acordeón) → FinalCTA → Footer → FloatingBar (móvil). Fondos alternados (steel-950 / gradiente / radial / rojo) para variar el ritmo.
