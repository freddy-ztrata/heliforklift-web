...
- **Secciones:** StickyHeader (aparece tras scroll) → Hero (**top bar con logo HELI siempre visible** + chips de energía clickeables + 1 máquina hero) → **FleetMarquee** (marquee infinito a todo el ancho con 8 máquinas de todos los tipos — comunica "todos los tipos") → TrustStrip (stats animadas con `CountUp` desde `company.stats` + certificaciones) → EnergyBento (4 tipos de energía desde `fuelTypeCategories`) → Categories (8 categorías de equipo) → WhyHeli (6 diferenciadores) → Services (3) → Process (3 pasos) → **Quoter** (form) → Coverage (sucursales desde `contact.locations`) → FAQ (acordeón) → FinalCTA → Footer → FloatingBar (móvil: Llamar + Cotizar).
